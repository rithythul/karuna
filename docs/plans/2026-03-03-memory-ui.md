...
**Goal:** Add a `/memory` page where users can see and delete the cross-task learnings Hanuman has accumulated about them.

**Architecture:** New Next.js page at `app/memory/page.tsx` that fetches from the existing `/api/memory` endpoint, groups entries by category (preference / fact / learning), and lets users delete individual entries via `DELETE /api/memory/{category}/{key}`. No backend changes needed — all endpoints exist.

**Tech Stack:** React 19, Next.js 16, TypeScript, Tailwind CSS 4, CSS custom properties

---

### Task 1: Create the Memory page
...
```bash
git add frontend/src/app/memory/page.tsx
git commit -m "feat: add /memory page for cross-task user memory management"
```

---

### Task 2: Add Memory nav link to home page and task history page

**Files:**
- Modify: `frontend/src/app/page.tsx`
- Modify: `frontend/src/app/tasks/page.tsx`

**Step 1: Home page — add Memory link**

In `page.tsx`, find the top-right nav (the `div` with `fixed top-4 right-4 z-50`). It currently has: "Task History" link, user name, "Sign out" button.

Add a Memory link immediately after the "Task History" link:
...
**Step 2: Task history page — add Memory link**

In `tasks/page.tsx`, find the `<header>` component. It has a title "Task History" on the left and a "New Task" button on the right.

Add a Memory link between the title and the New Task button (on the right side):
...
Place it inside the right-side container, before the "New Task" link.
...
- Empty state renders correctly if no memories exist
- Memory link appears in the nav on home and task history pages

**Step 5: Commit**

```bash
git add frontend/src/app/page.tsx frontend/src/app/tasks/page.tsx
git commit -m "feat: add Memory nav link to home and task history pages"
```
